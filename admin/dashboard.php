...
if (empty($_SESSION['username'])) {
  ...
  switch ($err) {
      case 'sukses':
        echo "<script>alert('Sukses Hapus!');window.location.href='/admin/';</script>";
          break;
      case 'gagal':
        echo "<script>alert('Gagal Hapus!');window.location.href='/admin/';</script>";
          break;
  }
  ?>
  <!doctype html>
    ...
    <div class="row flex-nowrap">
        <div class="col-auto col-md-3 col-xl-2 px-sm-2 px-0 bg-dark">
            <div class="d-flex flex-column align-items-center align-items-sm-start px-3 pt-2 text-white min-vh-100">
                ...
                <hr>
                <?php
                 $dbc = new mysqli("localhost", "root", "", "datasiswa-db");
                 $query = "SELECT * FROM db_login WHERE id = 1";
                ...
                <div class="dropdown pb-4 fixed-bottom">
                    <a href="#" class="d-flex align-items-center text-white text-decoration-none dropdown-toggle" id="dropdownUser1" data-bs-toggle="dropdown" aria-expanded="false">
                        ...
                        <span class="d-none d-sm-inline mx-1"><?=htmlspecialchars($row[1]);?></span>
                     <?php } ?>
                    </a>
        ...
        <div class="col py-3">
        <form class="d-flex col-md-3 col-xs-3" action="" method="GET">
...
<thead>
    <tr>
        ...
        <th scope="col">Action
        <a class="btn btn-success pt-1 pb-1" data-bs-toggle="modal" data-bs-target="#addModal" style="float:right;">
...
<tbody>
    ...
    include "config.php";
    $batas = 10;
    $q = mysqli_real_escape_string($dbc, htmlspecialchars(htmlentities(stripslashes($_REQUEST["q"]))));
    //echo $q;
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <div class="mb-2 col-md-5">
            <label for="message-text" class="col-form-label">Tanggal Lahir</label>
            <input type="date" class="form-control" name="ttl" placeholder="2002-12-31">
          </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="submit" class="btn btn-primary" onclick="return confirm('Yakin data sudah benar?');">Add Data</button>
            </form>
      </div>
    </div>
  </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="update_settings.php" method="POST">
            ...
            <input type="text" class="form-control" name="username" placeholder="<?=$row[1];?>">
          </div>
          <div class="mb-2 col-md-7">
            <label for="message-text" class="col-form-label">Password</label>
            <input type="password" class="form-control" name="pass" placeholder="<?=$row[2];?>">
          </div>
          <div class="mb-2 col-md-7">
            <label for="message-text" class="col-form-label">Confirm Password</label>
            <input type="password" class="form-control" name="cpass" placeholder="<?=$row[2];?>">
          </div>
          <?php
                 }
                 ?>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="submit" class="btn btn-primary" onclick="return confirm('Yakin data sudah benar?');">Update Profile</button>
        </form>
      </div>
    </div>
  </div>
